...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if( this != NULL)
    {
        if(nodeIndex >= 0 && nodeIndex <= ll_len(this))
        {
            ...
            if(nuevoNodo != NULL)// si hay lugar en memoria
            {
                //agrego el elemento al nodo
                nuevoNodo->pElement = pElement;
                //pongo en null el puntero al siguiente
                nuevoNodo->pNextNode = NULL;

                if(nodeIndex == 0)//si es en el primer lugar
                {
                    //tomo el puntero al primer nodo del linked list y lo guardo en el puntero al siguiente
                    nuevoNodo->pNextNode = this->pFirstNode;
                    //pongo el puntero al nuevo nodo como primer elemento del linked list
                    this->pFirstNode = nuevoNodo;
                }
                else
                {
                    //si no es el primer lugar ..
                    //guardo el puntero al primer nodo del linked list
                    prev = this->pFirstNode;
                    //guardo el puntero al siguiente nodo del linked list
                    next = prev->pNextNode;


                   // while( nodeIndex > 1)//itero hasta el indice pedido
                    while( nodeIndex >contador)//itero hasta el indice pedido
                    {
                          //guardo el puntero al  nodo del linked list
                        prev  = next;
                        //guardo el puntero al siguiente nodo del linked list
                        next  = prev->pNextNode;
                        //nodeIndex--;
